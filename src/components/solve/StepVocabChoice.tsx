...
interface Props {
  ...
  // ── 오답 회복(RecoveryLoop) wiring — ALL built in VocabWizard from the FULL
  // rubric; this component reads NO rubric and only passes these through. ──
  /** 원칙4 diagnostic questions (polarity axis), pre-bound to the wrong pick. */
  recoveryQuestions: RecoveryQuestion[]
  /** The single re-selection UI + its frame-closure grader. */
  reselect: RecoveryReselect
  /** 원칙2 mirror — the student's OWN prior 밑줄 진단 judgment. */
...
/**
 * 어휘 마지막 단계(self-drives) — 5개 밑줄 중 문맥과 안 맞는 낱말을 고르고, 왜
 * 그런지 근거 버튼 + 한 줄 근거를 남긴 뒤 제출한다. 정답 commit → 성공 배너 + 해설;
 * 오답 commit → <RecoveryLoop>(정답 비노출, 프레임 클로저로만 채점).
 * why_options는 정답 밑줄에만 있으므로, 정답 num을 골랐을 때만 버튼이 뜬다(의도됨).
 */
export default function StepVocabChoice({
  underlines,
  answer,
  explanation,
  choices,
    ...
    if (known && !correct) {
      return (
        <RecoveryLoop
          ...
          initialAnswer={`${selected}. ${selectedWord}`}
  ...
  function commit() {
    ...
    const finalState: VocabChoiceState = { ...value, finalChoice: picked }
    onChange(finalState)
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm font-semibold text-ink">
        문맥상 <span className="text-pink-600">어색한 낱말</span>은?
      </p>
      ...
      {picked !== null && reasonOptions.length > 0 && (
        <ReasonButtons
          question={`${picked}번 '${pickedWord}'가 어색하다고 봤네요. 왜요?`}
      ...
      <div>
        ...
        <input
          ...
          placeholder="예: 글 전체는 긍정인데 이 낱말만 부정 뜻이라 흐름과 반대예요"

...
create table if not exists chat_messages (
  ...
  model         text,                  -- 실측: 응답 생성 모델 (claude-sonnet-5 / gpt-5.1 / gpt-4o-mini)
  provider      text,                  -- 실측: 호출 경로 (gateway / openai) — model과 함께 provenance
  corpus_role   text check (corpus_role in ('pilot','research')) default 'pilot',
                                        -- 'research' = 진짜 sonnet(gateway) 수집분만. gpt/파일럿은 'pilot'
  input_mode    text,                  -- 학생 입력 방식 (button / fill_in) — 연구 메타
  created_at    timestamptz default now()
);
